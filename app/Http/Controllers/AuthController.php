...
class AuthController extends Controller
{
    ...
    {
        $user = DB::table('users')->where('nip', $request->id)->first();
        return response()->json($user);
    }
    public function deleteAkun(Request $request)
    {
        $deleted = DB::table('users')->where('nip', $request->input('idAkun'))->delete();
        if ($deleted == 0) {
    ...
    public function dashboard()
    {
        $user = session()->get('user');
        if ($user->role_id == 2) {
            $jumlahSM = DB::table('suratmasuk')->where('created_by', $user->nip)->count();
            $jumlahSK = DB::table('suratkeluar')->where('jenis', 'biasa')->where('status', 'digunakan')->where('created_by', $user->nip)->count();
            $jumlahSA = DB::table('suratkeluar')->where('jenis', 'antidatir')->where('status', 'digunakan')->where('created_by', $user->nip)->count();
        } else {
